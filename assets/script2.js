$(function () {
  const results = $('#results'), // Displays results
  ...
  $('#btn').on('click', (e) => {
    ...
    const apiKey = '6513fb31ba384f9391f06324986c1880',
      city = search.val(),
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        let cityID = data.city.id, // City ID
          cityName = data.city.name, // City Name
          todaysDate = dateConverter(data.list[0].dt_txt.slice(0, 10)), // Current Date
          currentTemp = fahrenheitConverter(data.list[0].main.temp), // Current Weather T
            ...
            <div class="row-card-1 row align-items-center shadow-lg" style="border-radius: 10px; border-style: solid; border-color: rgba(83,83,83,0.16)">
              ${[8, 16, 24, 32, 39]
                ...
                <div class="col" style="padding: 1rem;">
                  ...
                  }@2x.png" alt="weather icon" title="icon">
                  <br> Temp: ${fahrenheitConverter(
                    data.list[i].main.temp
                  )}&#176; <br> Wind: ${data.list[i].wind.speed} MPH
        ...
        if (!isDuplicate) {
          ...
          localStorage.setItem(
            `city-${id}`,
            JSON.stringify({ cityID, cityName, todaysDate })
          );
        }
      })
  ...
  historyEl.click((e) => {
    ...
    if (historyEl.children().length > 0) {
      ...
      fetch(url)
        .then((res) => res.json())
        .then((data) => {
          let cityName = data.city.name, // City Name
